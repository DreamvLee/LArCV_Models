...
class ASPP(nn.Module):
    def __init__(self, rate1, rate2, rate3, rate4, pad1, pad2, pad3, pad4, inplanes, outplanes=16, nkernels=16, showsizes=False):
        ...
        self.B1_conv = nn.Conv2d(self.inplanes, self.outplanes, kernel_size=1,
            stride=stride, padding=pad1, dilation=rate1, bias=True)
        ...
        self.B2_conv = nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3,
            stride=stride, padding=pad2, dilation=rate2, bias=True)
        ...
        self.B3_conv = nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3,
            stride=stride, padding=pad3, dilation=rate3, bias=True)
        ...
        self.B4_conv = nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3,
            stride=stride, padding=pad4, dilation=rate4, bias=True)
        self.B4_bn = nn.BatchNorm2d(self.nkernels)
        self.B4_relu = nn.ReLU(inplace=True)

        # Block 5
        self.B5_gp = nn.MaxPool2d(kernel_size=3, stride=stride, padding=1,
...
class ASPP_ResNet(nn.Module):
    ...
    def __init__(self, num_classes=3, in_channels=3, inplanes=16, showsizes=True):
        ...
        self.ASPP_layer_enc3 = self.ASPP_layer(rate1 = 1, rate2 = 8, rate3 = 16, rate4 = 32, pad1 = 0, pad2 = 8, pad3 = 16, pad4 = 32, inplanes = 128)

        self.ASPP_combine_enc3 = self.ASPP_combine(self.inplanes * 12, self.inplanes * 8)

        self.ASPP_layer_enc4 = self.ASPP_layer(self.inplanes * 16, rate1 = 1, rate2 = 4, rate3 = 8, rate4 = 16, pad1 = 0, pad2 = 4, pad3 = 8, pad4 = 16)

        self.ASPP_combine_enc4 = self.ASPP_combine(self.inplanes * 20, self.inplanes * 16)

        self.ASPP_layer_enc5 = self.ASPP_layer(self.inplanes * 32, rate1 = 1, rate2 = 2, rate3 = 4, rate4 = 8, pad1 = 0, pad2 = 2, pad3 = 4, pad4 = 8)
    ...
    def ASPP_layer(self, inplanes, rate1, rate2, rate3, rate4, pad1, pad2, pad3, pad4):
        return ASPP(inplanes, rate1, rate2, rate3, rate4, pad1, pad2, pad3, pad4)
