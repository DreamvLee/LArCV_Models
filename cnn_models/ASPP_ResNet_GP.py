...
class DoubleResNet(nn.Module):
    def __init__(self, inplanes, planes, stride=1, showsizes=False):
    ...
    def forward(self, x):
        ...
        if self.showsizes:
            print "x size before res1(x):", x.size()
...
class ConvTransposeLayer(nn.Module):
    def __init__(self, deconv_inplanes, deconv_outplanes, res_outplanes, showsizes=False):
    ...
    def forward(self, x, skip_x):
        ...
        if self.showsizes:
            print "skip_x size is:", skip_x.size()
            print "x size before deconv:", x.size()
        ...
        if self.showsizes:
            print "out after concat(out, skip_x):", out.size()
...
class ASPP(nn.Module):
    def __init__(self, inplanes, outplanes=16, nkernels=16, showsizes=False):
        ...
        # Block 2
        self.B2_conv = nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3, stride=stride, padding=2, dilation=2, bias=True)
        self.B2_bn = nn.BatchNorm2d(self.nkernels)
        self.B2_relu = nn.ReLU(inplace=True)

        # Block 3
        self.B3_conv = nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3, stride=stride, padding=4, dilation=4, bias=True)
        self.B3_bn = nn.BatchNorm2d(self.nkernels)
        self.B3_relu = nn.ReLU(inplace=True)

        # Block 4
        self.B4_conv = nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3, stride=stride, padding=6, dilation=6, bias=True)
...
class ASPP_ResNet(nn.Module):
    ...
    def __init__(self, num_classes=3, in_channels=3, inplanes=16, showsizes=True):
        ...
        # Decoding Layer
        self.dec_layer5 = self._make_decoding_layer(self.inplanes * 32, self.inplanes * 16, self.inplanes * 32)  # 512->256
    ...
    def forward(self, x):
        ...
        if self.showsizes:
            print "e4_ASPP after ASPP_Layer:", e4_ASPP.size()

        e4_ASPP = self.ASPP_combine_enc4(e4_ASPP)
        if self.showsizes:
            print "e4_ASPP after combine:", e4_ASPP.size()

        # e4_ASPP = self.BiUp(e4_ASPP)
        # if self.showsizes:
        #     print "e4_ASPP after BiUp:", e4_ASPP.size()

        e4_skip = torch.cat((e4_ASPP, e4), 1)

        if self.showsizes:
            print "e4_skip after concat:", e4_skip.size()

        e5_ASPP = self.ASPP_layer_enc5(e5)

        if self.showsizes:
            print "e5_ASPP after ASPP_Layer:", e5_ASPP.size()

        e5_ASPP = self.ASPP_combine_enc5(e5_ASPP)

        if self.showsizes:
            print "e5_ASPP after ASPP_Combine:", e5_ASPP.size()

        # Decoding Layer
        d5 = self.dec_layer5(e5_ASPP, e4_skip)
