...
# Model definition
from Atrous_Sep_Conv_Mod1 import Atrous_Sep_Conv

GPUMODE = True
GPUID = 1
RESUME_FROM_CHECKPOINT = False
RUNPROFILER = False
CHECKPOINT_FILE = "checkpoint.30000th.tar"
...
class PixelWiseNLLLoss(nn.modules.loss._WeightedLoss):
    def __init__(self, weight=None, size_average=True, ignore_index=-100):
        ...
        self.reduce = False
        nweight = np.ones((3), dtype=np.float32)  # upweight shower pixels
        nweight[2] = 1.0
...
def main():
    ...
    if GPUMODE:
        model = Atrous_Sep_Conv(inplanes=16, in_channels=1, num_classes=3, showsizes=False)
        model.cuda(GPUID)
    else:
        model = Atrous_Sep_Conv(inplanes=16, in_channels=1, num_classes=3)
