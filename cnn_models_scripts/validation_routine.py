...
RUNPROFILER             = False
CHECKPOINT_FILE         = "checkpoint_caffe_uresnet.17000th.tar"
NUM_VALIDATION_ROUTINES = 0
# TODO: Create automated validation routine
#       Put for loop inside main function
#       Have python write results to txt
#       Append to txt when loop completes
#       Remove raw_input()
##########################################
...
"""
    ...
    filename  = modelinfo + '{:%m_%d_%Y}' + '.csv'
    today     = pd.Timestamp('today')

    if os.path.isfile(filename.format(today)):
        with open(filename.format(today), 'a') as vD:
        ...
        with open(filename.format(today), 'w') as vD:
            valWriter = csv.writer(vD, delimiter=',')
            valWriter.writerow(['Model:                     ', modelName])
            valWriter.writerow(['Checkpoint File:           ', CHECKPOINT_FILE])
            valWriter.writerow(["Total pixel count:"])
